...
public class AuthorController {
	...
	public String findAuthorById(@PathVariable("id") Long id, Model model) {
		Author auth=authorService.findAuthorById(id);
		if(auth==null) {
			String a="Author with this id doesn't exists";
	...
	public String createAuthor(Author author, BindingResult result, Model model) {
		...
		List<Author> a = authorService.findAllAuthors();
		
		for (Author temp : a) {
			if (temp.getName().equals(author.getName())) 
			{
				Long id=temp.getId();
				author.setId(id);
