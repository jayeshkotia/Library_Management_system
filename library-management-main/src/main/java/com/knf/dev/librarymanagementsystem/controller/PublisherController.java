...
public class PublisherController {
	...
	public String findPublisherById(@PathVariable("id") Long id, Model model) {
		...
		Publisher pub=publisherService.findPublisherById(id);
		if(pub==null) {
			String a="Publisher with this id doesn't exists";
	...
	public String createPublisher(Publisher publisher, BindingResult result, Model model) {
		...
		List<Publisher> a = publisherService.findAllPublishers();

		for (Publisher temp : a) {
			if (temp.getName().equals(publisher.getName())) {
				Long id = temp.getId();
				publisher.setId(id);
